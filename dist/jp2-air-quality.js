/*
  ...
  - UX: en mode éditeur, le libellé “(capteurs masqués)” n’est plus affiché quand “Afficher les capteurs” est désactivé.
...
if (xy.length < 2) {
  ...
  _renderAQICard() {
    ...
    if (c.aqi_show_sensors === false) {
      // En mode éditeur, l'option "Afficher les capteurs" est souvent utilisée pour alléger la prévisualisation.
      // On évite donc d'afficher le libellé "(capteurs masqués)" dans la carte.
      const inEditor = !!(this._hass?.lovelace?.editMode);
      body = inEditor ? null : el("div", { style: { marginTop: "6px", opacity: ".65", fontSize: "12px" } }, ["(capteurs masqués)"]);
