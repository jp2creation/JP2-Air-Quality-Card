/*
  ...
  Release notes — v2.2.0 (current version)

  - Fix: Visual editor “Overrides” now shows the sensor/entity names again.
  - Fix: Overrides summary updates immediately when changing name/icon.
  - Fix: Visual editor no longer crashes (missing _schemaStatusLabels()).
  - Fix: “Status labels” section is now available for all presets in Sensor mode.
  - New: Term label overrides to rename common words (Good / Comfort / Low / Moderate / High / etc.).
  - New: Added CO and CH₂O presets (thresholds + units + icons).
  - New: Status level overrides (visual editor) to rename Rising/Band statuses.
  - Fix: Preset auto-detection now distinguishes CO vs CO₂ (both can be ppm) and detects CH₂O/HCHO.
  - Change: UI strings translated to English.
...
const CARD_BUILD_DATE = "2026-02-22";
// -------------------------
// Defaults / presets
// -------------------------
...
const DEFAULT_ICON_BY_PRESET = {
  ...
  co: "mdi:molecule",
  ch2o: "mdi:flask-outline",
};

const DEFAULT_TERM_LABELS = Object.freeze({
  good: "Good",
  ventilate: "Ventilate",
  comfort: "Comfort",
  watch: "Watch",
...
function jp2IsInEditor(el, hass) {
  try {
    const lovelace = hass?.lovelace;
    if (lovelace && typeof lovelace.editMode === "boolean") return lovelace.editMode;
  } catch (_) {}
  try {
    if (el && typeof el.closest === "function") {
      const hit = el.closest("hui-card-preview, ha-card-preview, hui-card-element-editor, hui-card-editor, ha-panel-lovelace-editor");
      if (hit) return true;
    }
  } catch (_) {}
  return false;
...
class Jp2AirQualityCard extends HTMLElement {
  constructor() {
    ...
    // AQI: capteur “limitant” (pour default more-info)
    this._aqiWorstEntityId = null;
    // visualizer state
    this._viz = {
      open: false,
      hours: null,
  ...
  static getConfigForm(config = {}) {
    ...
    const cardTypeOptions = [
      { label: "Sensor card", value: "sensor" },
      { label: "AQI card (multi-sensors)", value: "aqi" },
    ];
    ...
    const presetOptions = [
      ...
      { label: "VOC / TVOC", value: "voc" },
      { label: "PM1", value: "pm1" },
      { label: "PM2.5", value: "pm25" },
      { label: "CO₂", value: "co2" },
      { label: "CO", value: "co" },
      { label: "CH₂O", value: "ch2o" },
      { label: "Custom (free sensor)", value: "custom" },
    ];

    const graphPosOptions = [
      { label: "Below header", value: "below_top" },
      { label: "Inside header (below the bar)", value: "inside_top" },
      { label: "Top", value: "top" },
    ...
    const graphColorModeOptions = [
      ...
      { label: "Orange/red peaks", value: "peaks" },
      { label: "Orange/red segments", value: "segments" },
    ];
    ...
    const sensorSchema = [
      ...
      },
      {
        ...
        schema: [
          ...
          { name: "knob_color_mode", selector: { select: { options: [
            ...
            { label: "Status color", value: "status" },
          ], mode: "dropdown" } } },
      ...
      },
      {
        type: "expandable",
        name: "graph",
        title: "Internal graph (Home Assistant history)",
        flatten: true,
        schema: [
          {
            type: "grid",
  ...
  schema: [
    {
      ...
      column_min_width: "240px",
      schema: [
        { name: "status_labels.rising_good", selector: { text: {} } },
        { name: "status_labels.rising_warn", selector: { text: {} } },
        { name: "status_labels.rising_bad", selector: { text: {} } },
        { name: "status_labels.band_good", selector: { text: {} } },
        { name: "status_labels.band_warn", selector: { text: {} } },
        { name: "status_labels.band_bad", selector: { text: {} } },
      ],
    },
  ],
},
  ...
  schema: [
    {
      ...
      column_min_width: "240px",
      schema: [
        { name: "term_labels.good", selector: { text: {} } },
        { name: "term_labels.ventilate", selector: { text: {} } },
        { name: "term_labels.comfort", selector: { text: {} } },
        { name: "term_labels.watch", selector: { text: {} } },
        { name: "term_labels.alert", selector: { text: {} } },
        { name: "term_labels.low", selector: { text: {} } },
        { name: "term_labels.moderate", selector: { text: {} } },
        { name: "term_labels.high", selector: { text: {} } },
        { name: "term_labels.bad", selector: { text: {} } },
        { name: "term_labels.discomfort", selector: { text: {} } },
        { name: "term_labels.normal", selector: { text: {} } },
        { name: "term_labels.variable", selector: { text: {} } },
        { name: "term_labels.extreme", selector: { text: {} } },
      ],
    },
  ],
},
  ...
  schema: [
    ...
    const labelMap = {
      ...
      preset: "Preset",
// custom preset
      "custom_preset.type": "Preset type",
      "custom_preset.unit_fallback": "Unit (fallback)",
"custom_preset.decimals": "Decimals",
"custom_preset.min": "Min value",
"custom_preset.max": "Max value",
"custom_preset.good_max": "Good threshold (max)",
"custom_preset.warn_max": "Warning threshold (max)",
"custom_preset.warn_low_min": "Low warning threshold (min)",
"custom_preset.good_min": "Good threshold (min)",
"custom_preset.good_max_band": "Good threshold (max, band)",
"custom_preset.warn_high_max": "High warning threshold (max)",
"custom_preset.label_good": "Good label",
"custom_preset.label_warn": "Warning label",
"custom_preset.label_bad": "Bad label",
"status_labels.rising_good": "Rising: Good",
"status_labels.rising_warn": "Rising: Warning",
"status_labels.rising_bad": "Rising: Bad",
"status_labels.band_good": "Band: Good",
"status_labels.band_warn": "Band: Warning",
"status_labels.band_bad": "Band: Bad",

"term_labels.good": "Term: Good",
"term_labels.ventilate": "Term: Ventilate",
"term_labels.comfort": "Term: Comfort",
"term_labels.watch": "Term: Watch",
"term_labels.alert": "Term: Alert",
"term_labels.low": "Term: Low",
"term_labels.moderate": "Term: Moderate",
"term_labels.high": "Term: High",
"term_labels.bad": "Term: Bad",
"term_labels.discomfort": "Term: Discomfort",
"term_labels.normal": "Term: Normal",
"term_labels.variable": "Term: Variable",
"term_labels.extreme": "Term: Extreme",


      name: "Name",
      icon: "Icon",
      background_enabled: "Colored background",
      bar_enabled: "Show bar",
      show_graph: "Show graph",
      graph_position: "Graph position",
      hours_to_show: "Hours to show",
      graph_height: "Graph height",
      line_width: "Line width",
      graph_color_mode: "Graph color mode",
      graph_color: "Graph color",
      graph_warn_color: "Warning color",
      graph_bad_color: "Bad color",
      show_title: "Show title",
      show_secondary: "Show attributes (line 2)",
      show_secondary_value: "Line 2: value",
      show_secondary_unit: "Line 2: unit",
      show_secondary_state: "Line 2: status",
      show_icon: "Show icon",
      show_value: "Show value (right)",
      show_unit: "Show unit (right)",
      show_knob: "Show indicator",
      knob_size: "Indicator size (px)",
      knob_outline: "Indicator outline",
      knob_shadow: "Indicator shadow",
      icon_size: "Icon size (circle)",
      icon_inner_size: "Pictogram size",
      icon_background: "Icon background",
      icon_circle: "Icon circle",
      title_size: "Title size",
      value_size: "Value size (right)",
      unit_size: "Unit size (right)",
      secondary_value_size: "Line 2 value size",
      secondary_unit_size: "Line 2 unit size",
      secondary_state_size: "Line 2 status size",
      good: "Green color",
      warn: "Orange color",
      bad: "Red color",
      width: "Width (%)",
      ...
      align: "Alignment",
      opacity: "Opacity (%)",
      knob_outline_size: "Marker outline size (px)",
      knob_color_mode: "Marker color",
      opacity: "Bar opacity (%)",
      aqi_title: "Title AQI",
      aqi_title_icon: "Title icon (AQI)",
      aqi_entities: "AQI entities",
      aqi_show_title: "Show title",
      aqi_show_global: "Show global status",
      aqi_air_only: "Air only",
      aqi_show_sensors: "Show sensor list",
      aqi_background_enabled: "Colored background (AQI)",
    };

    const helperMap = {
      aqi_entities: "Add multiple sensors (CO₂, CO, CH₂O, VOC/TVOC, PM2.5, Radon...). A status is computed for each.",
      aqi_title_icon: "Optional: an icon displayed to the left of the title in AQI mode.",
      aqi_air_only: "If enabled, the global status ignores temperature/humidity/pressure and only considers CO₂/CO/CH₂O/VOC/PM/Radon.",
      graph_color: "Example: var(--primary-color) or #00bcd4. Empty = theme color.",
      graph_warn_color: "Empty = bar warning color.",
      graph_bad_color: "Empty = bar bad color.",
    };
  ...
  setConfig(config) {
    ...
    merged.status_labels = merged.status_labels && typeof merged.status_labels === "object" ? merged.status_labels : {};
    merged.term_labels = merged.term_labels && typeof merged.term_labels === "object" ? merged.term_labels : {};
...
if (p === "custom") {
  ...
  good_max: 50,
  warn_max: 80,
  // Band preset thresholds (only used when type === "band")
...
const map = {
  ...
  radon: { ...common,
    ...
    min: 0, max: 400,          // extended to show the red zone (>=300)
    good_max: 99, warn_max: 299,
    label_good: "Good", label_warn: "Moderate", label_bad: "Bad",
  },
  ...
  co2: { ...common,
    ...
    label_good: "Good", label_warn: "Ventilate", label_bad: "High",
  },

  co: { ...common,
    type: "rising", decimals: 0, unit_fallback: "ppm",
    min: 0, max: 50,
    good_max: 9, warn_max: 35,
    label_good: "Good", label_warn: "Ventilate", label_bad: "High",
  },

  ch2o: { ...common,
    type: "rising", decimals: 3, unit_fallback: "mg/m³",
    min: 0, max: 0.3,
    good_max: 0.05, warn_max: 0.1,
    label_good: "Good", label_warn: "Ventilate", label_bad: "High",
  },
  ...
  voc: { ...common,
    ...
    label_good: "Low", label_warn: "Ventilate", label_bad: "Very high",
  },
  ...
  temperature: { ...common,
    ...
    good_max_band: 24,
    warn_high_max: 26,
    label_good: "Comfort", label_warn: "Watch", label_bad: "Alert",
  },
  ...
  humidity: { ...common,
    ...
    good_max_band: 60,
    warn_high_max: 70,
    label_good: "Comfort", label_warn: "Watch", label_bad: "Discomfort",
  },
  ...
  pressure: { ...common,
    ...
    good_max_band: 1030,
    warn_high_max: 1040,
    label_good: "Normal", label_warn: "Variable", label_bad: "Extreme",
  },
};
;
...
_applyStatusLabelOverride(presetType, level, label) {
  try {
    const ov = this._config?.status_labels;
    if (!ov || typeof ov !== "object") return label;

    const type = (String(presetType) === "band") ? "band" : "rising";
    const lvl = String(level || "");

    let key = "";
    if (type === "band") {
      key = (lvl === "good") ? "band_good" : (lvl === "warn") ? "band_warn" : (lvl === "bad") ? "band_bad" : "";
    } else {
      key = (lvl === "good") ? "rising_good" : (lvl === "warn") ? "rising_warn" : (lvl === "bad") ? "rising_bad" : "";
    }
    if (!key) return label;

    const v = String(ov[key] ?? "").trim();
    return v ? v : label;
  } catch (_) {
    return label;
  }
  ...
  _applyTermLabelOverride(label) {
    try {
      const ov = this._config?.term_labels;
      if (!ov || typeof ov !== "object") return label;

      const src = String(label ?? "");
      if (!src) return label;

      // Direct mapping by exact label (YAML friendly: term_labels: { "Comfort": "Confort" })
      if (Object.prototype.hasOwnProperty.call(ov, src)) {
        const v = String(ov[src] ?? "").trim();
        if (v) return v;
      }

      const lower = src.toLowerCase();
      for (const [k, def] of Object.entries(DEFAULT_TERM_LABELS)) {
        if (String(def).toLowerCase() === lower) {
  ...
  _statusFor(preset, value) {
    ...
    if (String(pc.type) === "band") {
      ...
      label = this._applyStatusLabelOverride(pc.type, level, label);
      label = this._applyTermLabelOverride(label);
  ...
  _detectPreset(entityId, stateObj) {
    ...
    if (
      dc.includes("formaldehyde") ||
      hay.includes("ch2o") || hay.includes("hcho") || hay.includes("formalde")
    ) return "ch2o";

    // CO2
    if (dc.includes("carbon_dioxide") || hay.includes("co2")) return "co2";

    // CO (carbon monoxide)
    if (
      dc.includes("carbon_monoxide") ||
      /(^|[._\-])co($|[._\-])/.test(id) ||
      /\bco\b/.test(name) ||
      name.includes("carbon monoxide")
    ) return "co";

    // ppm fallback: most sensors are CO2 in ppm, but we already handled CO above
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="viz-overlay" id="vizOverlay" aria-hidden="true">
        <div class="viz" role="dialog" aria-modal="true" aria-label="Visualisateur d'historique">
          <div class="viz-head">
            <div class="viz-title">
              <div class="t"><span class="dot" id="vizDot"></span><span id="vizTitle">History</span></div>
...
// Visualizer elements
  ...
  _handleCardGesture(gesture, ev) {
    ...
    if (String(c.card_mode || "") === "aqi") {
      if (this._aqiWorstEntityId) return this._aqiWorstEntityId;
      const ents = Array.isArray(c.aqi_entities) ? c.aqi_entities : [];
      if (ents.length) return ents[0];
    }
  ...
  _openVisualizer(ctx) {
    ...
    const title = String(ctx?.title || c.name || DEFAULT_NAME_BY_PRESET[preset] || "History");
  ...
  async _renderVisualizer() {
    ...
    if (this._vizControlsEl) {
      ...
      for (const h of ranges) {
        const chip = el("div", {
          ...
          role: "button",
          tabindex: "0",
          title: `Show ${jp2FormatHourLabel(h)}`,
          onclick: () => {
            this._viz.hours = h;
            this._renderVisualizer();
          },
      ...
      addToggle("Smooth", !!this._viz.smooth, () => { this._viz.smooth = !this._viz.smooth; this._renderVisualizer(); });
      addToggle("Thresholds", !!this._viz.showThresholds, () => { this._viz.showThresholds = !this._viz.showThresholds; this._renderVisualizer(); });
      addToggle("Stats", !!this._viz.showStats, () => { this._viz.showStats = !this._viz.showStats; this._renderVisualizer(); });
    }

    // Loading state
    if (this._vizChartEl) this._vizChartEl.innerHTML = `<div class="msg" style="padding:10px; font-size:12px; opacity:.75;">Loading…</div>`;
    ...
    if (!rawPoints || rawPoints.length < 2) {
      if (this._vizChartEl) this._vizChartEl.innerHTML = `<div class="msg" style="padding:10px; font-size:12px; opacity:.75;">History unavailable</div>`;
      return;
    }
    ...
    const svg = `
      <svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-label="Detailed history">
        <defs>
  ...
  async _renderInternalGraph(entityId, preset) {
    ...
    graph.innerHTML = `<div class="msg">Loading history…</div>`;
    const points = await this._getHistoryPoints(entityId, hours);

    if (!points || points.length < 2) {
      graph.innerHTML = `<div class="msg">History unavailable</div>`;
      return;
    }
...
if (xy.length < 2) {
    ...
    const svg = `
      <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" aria-label="History">
        ${svgParts.join("")}
      </svg>
    `;
  ...
  _renderAQICard() {
    ...
    // Option "Air only" : ignore temperature/humidity/pressure, etc.
    const levelRank = { unknown: -1, good: 0, warn: 1, bad: 2 };
    const airOnly = !!c.aqi_air_only;
    const AIR_PRESETS = new Set(["co2", "co", "ch2o", "voc", "pm1", "pm25", "radon"]);
    ...
    // Pour les actions Lovelace (more-info) en AQI
    this._aqiWorstEntityId = worst?.eid || null;
    ...
    const inEditor = jp2IsInEditor(this, this._hass);

    let body = null;
    if (c.aqi_show_sensors === false) {
      if (!inEditor) body = el("div", { style: { marginTop: "6px", display: "none", opacity: ".65", fontSize: "12px" } }, ["(sensors hidden)"]);
      ...
      body = el("div", { class: "tiles", "data-jp2-no-card-action": "1" }, rows.map((r) => {
      ...
      body = el("div", { class: "aqi-list", "data-jp2-no-card-action": "1" }, rows.map((r) => {
  ...
  _ensureUI() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .ov-chev { opacity: .65; font-size: 18px; line-height: 1; transition: transform .16s ease; padding-top: 2px; }
        .ov.open .ov-chev { transform: rotate(180deg); }
.ov-pill {
          padding: 4px 8px;
          border-radius: 999px;
      ...
      <div class="wrap">
        <div class="hero">
          <div class="hero-top">
            <div>
              ...
              <div class="subtitle">Visual editor v${CARD_VERSION}</div>
            </div>
        ...
        <div class="footer">Tip: if you prefer YAML, you can switch the card editor to code mode.</div>
      </div>
    `;
...
// Tabs dépendants du mode
    ...
    modeText.textContent = isAqi ? "AQI (multi-sensors)" : "Sensor (1 entity)";
  ...
  _buildTabs() {
    if (this._isAqi) {
      return [
        ...
        { id: "aqi_entities", label: "Entities" },
        { id: "aqi_layout", label: "Layout" },
        { id: "aqi_icons", label: "Content" },
  ...
  _renderTabContent(tabId) {
    ...
    if (!isAqi) {
      if (tabId === "general") {
        ...
        "Configuration",
        "Choose the card type and entity. The preset automatically sets units and thresholds.",
        ...
        if (String(this._config?.preset || "") === "custom") {
          root.appendChild(this._section(
            "Custom preset",
            "Define your thresholds, unit, decimals and labels. (For \"Comfort band\", use warn_low_min / good_min / good_max_band / warn_high_max.)",
        ...
        root.appendChild(this._section(
          "Status labels",
          "Optional: override the status text shown on the card (Good / Ventilate / Comfort, etc.). Leave empty to keep the preset defaults.",
          this._makeForm(this._schemaStatusLabels(), this._config),
          "common.status_labels"
        ));

        root.appendChild(this._section(
          "Term labels",
          "Optional: rename common words used by presets (Good / Comfort / Low / Moderate / High, etc.). Leave empty to keep defaults.",
          this._makeForm(this._schemaTermLabels(), this._config),
          "common.term_labels"
        ));
        ...
        root.appendChild(this._section(
          "Interactions",
          "Standard Lovelace actions: tap, hold, double tap.",
...
return root;
      ...
      if (tabId === "display") {
        root.appendChild(this._section(
        "Display",
        "Show/hide blocks and adjust sizes (icons, typography, knob).",
...
return root;
      ...
      if (tabId === "graph") {
        root.appendChild(this._section(
        "Internal graph",
        "Light history (no mini-graph-card). Adjust height, hours and color mode.",
...
return root;
      ...
      if (tabId === "bar") {
        root.appendChild(this._section(
        "Colored bar",
        "Bar settings (width, height, alignment) + marker (circle).",
...
return root;
      ...
      if (tabId === "colors") {
        root.appendChild(this._section(
        "Colors",
        "Good / Warning / Bad colors (used for the bar, icon and status).",
...
return root;
    ...
    if (tabId === "general") {
      root.appendChild(this._section(
        "AQI — General",
        "Multi-sensor card: title, global status, background, etc.",
        this._makeForm(this._schemaAqiGeneral(), this._config)
      ,
        "aqi.general"
      ));
root.appendChild(this._section(
        "Global status — SVG icon",
        "Optional: show an SVG icon above the global status (good/warn/bad). Colors follow status or custom, with optional circle and background.",
...
root.appendChild(this._section(
        "Global status — Style",
        "Dot (size/outline) and text settings. You can hide the status while keeping the SVG.",
...
return root;
    ...
    if (tabId === "aqi_entities") {
      root.appendChild(this._section(
        "AQI — Entities",
        "Select your sensors (order = display order).",
...
return root;
    ...
    if (tabId === "aqi_layout") {
      root.appendChild(this._section(
        "AQI — Layout",
        "Choose the layout (vertical list or horizontal tiles) + tile options.",
...
return root;
    ...
    if (tabId === "aqi_icons") {
      root.appendChild(this._section(
        "AQI — Icons & content",
        "Control what is shown per sensor + icon style.",
...
root.appendChild(this._section(
        "Icon style",
        "Circle size, pictogram size, background and circle.",
...
return root;
    ...
    if (tabId === "aqi_overrides") {
      root.appendChild(this._section(
        ...
        "Override the name and/or icon for specific entities. (Optional)",
...
return root;
  ...
  _actionsEditorBody() {
    ...
    wrap.appendChild(mk("Tap action", "tap_action", "more-info"));
    wrap.appendChild(mk("Hold action", "hold_action", "more-info"));
...
"term_labels.extreme": "Term: Extreme",
      ...
      bar_enabled: "Show bar",
      show_graph: "Show graph",
      show_top: "Show header",
      show_title: "Show title",
      show_secondary: "Show line 2",
      show_secondary_value: "Line 2: value",
      show_secondary_unit: "Line 2: unit",
      show_secondary_state: "Line 2: status",
      show_icon: "Show icon",
      ...
      show_unit: "Show unit",
      show_knob: "Show marker",
      knob_size: "Marker size (px)",
      knob_outline: "Marker outline",
      knob_shadow: "Marker shadow",
      knob_outline_size: "Marker outline size (px)",
      knob_color_mode: "Marker color",
      icon_size: "Icon size (px)",
      icon_inner_size: "Pictogram size (px)",
      icon_background: "Icon background",
      icon_circle: "Icon circle",
      // typography
      title_size: "Title size (px)",
      value_size: "Value size (px)",
      unit_size: "Unit size (px)",
      secondary_value_size: "Line 2 value size (px)",
      secondary_unit_size: "Line 2 unit size (px)",
      secondary_state_size: "Line 2 status size (px)",
      // graph
      graph_position: "Graph position",
      hours_to_show: "Hours shown",
      graph_height: "Graph height (px)",
      line_width: "Line width",
      graph_color_mode: "Color mode",
      graph_color: "Line color",
      graph_warn_color: "Warning color",
      graph_bad_color: "Bad color",
      visualizer_enabled: "Visualizer (full screen)",
      visualizer_ranges: "Quick ranges (hours)",
      visualizer_show_stats: "Show stats (visualizer)",
      visualizer_show_thresholds: "Show thresholds (visualizer)",
      visualizer_smooth_default: "Smooth by default",
      // bar colors
      good: "Good (color)",
      warn: "Warning (color)",
      bad: "Bad (color)",
      ...
      // AQI general
      aqi_title: "Title",
      aqi_title_icon: "Title icon",
      aqi_show_title: "Show title",
      aqi_show_global: "Show global status",
      aqi_air_only: "Air only",
      aqi_show_sensors: "Show sensors",
      aqi_background_enabled: "Colored background",
      // AQI global SVG
      aqi_global_svg_enabled: "SVG icon above the status",
      aqi_global_svg_position: "SVG position",
      aqi_global_svg_align: "SVG alignment",
      aqi_global_svg_size: "SVG size (px)",
      aqi_global_svg_color_mode: "SVG color",
      aqi_global_svg_color: "SVG color (custom)",
      aqi_global_svg_show_icon: "Show SVG",
      aqi_global_svg_background: "Background (circle)",
      aqi_global_svg_background_opacity: "Background opacity (%)",
      aqi_global_svg_background_color_mode: "Background color",
      aqi_global_svg_background_color: "Background color (custom)",
      aqi_global_svg_circle: "Circle (outline)",
      aqi_global_svg_circle_width: "Circle stroke (px)",
      aqi_global_svg_circle_color_mode: "Circle color",
      aqi_global_svg_circle_color: "Circle color (custom)",
      // AQI global status
      aqi_global_status_enabled: "Show status (dot + text)",
      aqi_global_status_show_dot: "Show dot",
      aqi_global_status_show_text: "Show text",
      aqi_global_status_dot_size: "Dot size (px)",
      aqi_global_status_dot_outline: "Dot outline (px)",
      aqi_global_status_text_size: "Text size (px)",
      aqi_global_status_text_weight: "Text weight",
      // AQI entities / layout
      aqi_entities: "Entities (sensors)",
      aqi_layout: "Layout",
      aqi_tiles_per_row: "Tiles per row",
      aqi_tile_color_enabled: "Colored background per tile",
      aqi_tile_transparent: "Transparent background per tile",
      aqi_tile_outline_transparent: "Transparent outline per tile",
      aqi_tile_radius: "Tile rounding (px)",
      // AQI rows
      aqi_tiles_icons_only: "Horizontal: icons only",
      aqi_show_sensor_icon: "Show icon",
      aqi_show_sensor_name: "Show name",
      aqi_show_sensor_entity: "Show entity",
      aqi_show_sensor_value: "Show value",
      aqi_show_sensor_unit: "Show unit",
      aqi_show_sensor_status: "Show status",
      // AQI typography
      aqi_text_name_size: "Name: size (px)",
      aqi_text_name_weight: "Name: weight",
      aqi_text_value_size: "Value: size (px)",
      aqi_text_value_weight: "Value: weight",
      aqi_text_unit_size: "Unit: size (px)",
      aqi_text_unit_weight: "Unit: weight",
      aqi_text_status_size: "Status: size (px)",
      aqi_text_status_weight: "Status: weight",
      // AQI icons
      aqi_icon_size: "Icon size (circle)",
      aqi_icon_inner_size: "Pictogram size",
      aqi_icon_background: "Icon background",
      aqi_icon_circle: "Icon circle",
      aqi_icon_color_mode: "Icon color mode",
    };
  ...
  _computeHelper(s) {
    ...
    const key = n.startsWith("bar.") ? n.slice(4) : n;
    const map = {
"custom_preset.type": '"Thresholds" = higher is worse; "Comfort band" = good in the middle.',
"custom_preset.unit_fallback": "Used if the sensor does not provide a unit.",
"custom_preset.decimals": "Number of decimals to display.",
"custom_preset.min": "Scale minimum.",
"custom_preset.max": "Scale maximum.",
"custom_preset.good_max": 'Type "Thresholds": good if value ≤ good_max.',
"custom_preset.warn_max": 'Type "Thresholds": warning if value ≤ warn_max, else bad.',
"custom_preset.warn_low_min": 'Type "Comfort band": low warning (min).',
"custom_preset.good_min": 'Type "Comfort band": good (min).',
"custom_preset.good_max_band": 'Type "Comfort band": good (max).',
"custom_preset.warn_high_max": 'Type "Comfort band": high warning (max).',
"status_labels.rising_good": "Optional override for the displayed status text (Good).",
"status_labels.rising_warn": "Optional override for the displayed status text (Warning / Ventilate).",
"status_labels.rising_bad": "Optional override for the displayed status text (Bad).",
"status_labels.band_good": "Optional override for the displayed status text (Good / Comfort).",
"status_labels.band_warn": "Optional override for the displayed status text (Warning).",
"status_labels.band_bad": "Optional override for the displayed status text (Bad).",
"term_labels.good": "Optional: replace the word “Good” (used by some presets).",
"term_labels.ventilate": "Optional: replace the word “Ventilate”.",
"term_labels.comfort": "Optional: replace the word “Comfort”.",
"term_labels.watch": "Optional: replace the word “Watch”.",
"term_labels.alert": "Optional: replace the word “Alert”.",
"term_labels.low": "Optional: replace the word “Low”.",
"term_labels.moderate": "Optional: replace the word “Moderate”.",
"term_labels.high": "Optional: replace the word “High”.",
"term_labels.bad": "Optional: replace the word “Bad”.",
"term_labels.discomfort": "Optional: replace the word “Discomfort”.",
"term_labels.normal": "Optional: replace the word “Normal”.",
"term_labels.variable": "Optional: replace the word “Variable”.",
"term_labels.extreme": "Optional: replace the word “Extreme”.",

      graph_color: "Ex: #03a9f4 (laisse vide pour auto).",
      graph_warn_color: "Color for the warning area (peaks/segments).",
      graph_bad_color: "Color for the bad area (peaks/segments).",
      visualizer_enabled: "If enabled, tapping the mini-graph opens a full-screen visualizer.",
      visualizer_ranges: "List of quick ranges (in hours), e.g. 6,12,24,72,168.",
      visualizer_show_stats: "Show stats (min/max/avg/Δ) below the visualizer graph.",
      visualizer_show_thresholds: "Show threshold lines (good/warn) in the visualizer.",
      visualizer_smooth_default: "Apply light smoothing by default in the visualizer.",
      good: "Color used when status is good.",
      warn: "Color used when status is warning.",
      bad: "Color used when status is bad.",
      opacity: "Bar opacity in % (100 = opaque, 0 = invisible).",
      knob_color_mode: "Marker color: theme (primary color) or status (good/warn/bad).",
      knob_outline_size: "Marker outline width (when \"Marker outline\" is enabled).",
      aqi_entities: "You can select multiple entities.",
      aqi_title_icon: "Optional: an icon displayed to the left of the title in AQI mode.",
      aqi_air_only: "If enabled, the global status ignores temperature/humidity/pressure. Only CO₂, CO, CH₂O, VOC/TVOC, PM1/PM2.5 and Radon are considered.",
      aqi_tile_transparent: "If enabled, remove tile background (outline only).",
      aqi_tile_outline_transparent: "If enabled, also remove tile borders (no outline).",
      aqi_tiles_icons_only: "Only in horizontal layout: show only the icon for each sensor.",
      aqi_icon_color_mode: "Pictogram: colored (status color) or transparent (theme color).",
      aqi_global_svg_enabled: "Show an SVG icon above the global status (good/warn/bad).",
      aqi_global_svg_position: "Position: global = right (above the status); center = centered at the top of the card.",
      aqi_global_svg_align: "Horizontal SVG alignment (left/center/right).",
      aqi_global_svg_size: "Icon size (px).",
      aqi_global_svg_color_mode: "SVG color: status or custom.",
      aqi_global_svg_color: "CSS color (e.g. #ff0000). Leave empty for auto.",
      aqi_global_svg_show_icon: "Hide only the SVG (keep circle/background if enabled).",
      aqi_global_svg_background: "Background circle behind the SVG.",
      aqi_global_svg_background_opacity: "Background opacity (0..100).",
      aqi_global_svg_background_color_mode: "Background color: status or custom.",
      aqi_global_svg_background_color: "CSS background color. Leave empty for auto.",
      aqi_global_svg_circle: "Outline of the circle around the SVG.",
      aqi_global_svg_circle_width: "Outline width (0 = none).",
      aqi_global_svg_circle_color_mode: "Outline color: status or custom.",
      aqi_global_svg_circle_color: "CSS outline color. Leave empty for auto.",
      aqi_global_status_enabled: "Show the global status (dot + text). Disable it to keep only the SVG.",
      aqi_global_status_show_dot: "Show/hide the dot marker.",
      aqi_global_status_show_text: "Show/hide the status text.",
      aqi_global_status_dot_size: "Dot size (px).",
      aqi_global_status_dot_outline: "Dot outline width (px). 0 = none.",
      aqi_global_status_text_size: "Status text size (px). 0 = auto.",
      aqi_global_status_text_weight: "Text weight. 0 = auto.",
      aqi_text_name_size: "0 = auto (default size).",
      aqi_text_name_weight: "0 = auto. Common values: 400 / 600 / 700 / 800 / 900.",
...
_schemaCustomPreset() {
  return [
    { name: "custom_preset.type", selector: { select: { options: [
      { label: "Thresholds (higher is worse)", value: "rising" },
      { label: "Comfort band (middle is good)", value: "band" },
    ], mode: "dropdown" } } },
  ...
  _schemaAqiGlobalSvg() {
    ...
    const positionOptions = [
      { label: "Right (global block)", value: "global" },
      { label: "Centered on card", value: "center" },
    ];
    const alignOptions = [
      { label: "Left", value: "left" },
      { label: "Center", value: "center" },
  ...
  _aqiEntitiesReorder() {
    ...
    if (ents.length < 2) {
      ...
      muted.textContent = "Add at least 2 entities to enable reordering.";
  ...
  _schemaAqiLayout() {
    return [
      { name: "aqi_layout", selector: { select: { options: [
        ...
        { label: "Horizontal (tiles)", value: "horizontal" },
      ], mode: "dropdown" } } },
  ...
  _schemaAqiIconStyle() {
    return [
      { name: "aqi_icon_color_mode", selector: { select: { options: [
        { label: "Colored (status)", value: "colored" },
        { label: "Transparent (theme)", value: "transparent" },
      ], mode: "dropdown" } } },
...
this._renderOverridesInto(wrap);
  ...
  _renderOverridesInto(wrap) {
    ...
    if (!ents.length) {
      wrap.innerHTML = `<div class="muted">Add entities in the “Entities” tab to configure overrides.</div>`;
      return;
    }
    ...
    for (const eid of ents) {
      ...
      const displayName = (ov.name && String(ov.name).trim()) ? String(ov.name).trim() : name;

      const left = document.createElement("div");
      left.className = "ov-left";
      left.innerHTML = `
        <div class="ov-name">${_jp2EscapeHtml(displayName)}</div>
        <div class="ov-eid">${_jp2EscapeHtml(eid)}</div>
      `;

      const right = document.createElement("div");
      right.className = "ov-right";

      const pillName = document.createElement("div");
      pillName.className = "ov-pill";
      pillName.dataset.pill = "name";
      pillName.textContent = ov.name ? "Name" : "Auto name";

      const pillIcon = document.createElement("div");
      pillIcon.className = "ov-pill";
      pillIcon.dataset.pill = "icon";
      pillIcon.textContent = ov.icon ? "Icon" : "Auto icon";

      const chev = document.createElement("div");
      chev.className = "ov-chev";
      chev.textContent = "▾";

      right.appendChild(pillName);
      right.appendChild(pillIcon);
      ...
      const iconField = this._iconField("Icon", ov.icon || "");
  ...
  _updateOverrideRowUI(eid) {
    try {
      ...
      const st = this._hass?.states?.[eid];
      const baseName = st?.attributes?.friendly_name || eid;
      const nameEl = det.querySelector(".ov-name");
      if (nameEl) nameEl.textContent = (ov.name && String(ov.name).trim()) ? String(ov.name).trim() : baseName;

      const pillName = det.querySelector('.ov-pill[data-pill="name"]');
      const pillIcon = det.querySelector('.ov-pill[data-pill="icon"]');
      if (pillName) pillName.textContent = ov.name ? "Name" : "Auto name";
      if (pillIcon) pillIcon.textContent = ov.icon ? "Icon" : "Auto icon";
    } catch (_) {}
  }
